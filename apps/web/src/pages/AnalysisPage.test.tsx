...
test('主播汇总每个数值列常显升降序快捷键并可按时均成交排序', async () => {
  ...
  const ascending = await screen.findByRole('button', { name: '按时均成交升序' })
  ...
  expect(screen.getByRole('group', { name: '时均成交排序' })).toBeInTheDocument()

  fireEvent.click(ascending)
  expect(ascending).toHaveAttribute('aria-pressed', 'true')
  expect(ascending).toHaveAttribute('title', '取消时均成交升序')
  expect(ascending.closest('th')).toHaveClass('analysis-sorted-column')
  expect(document.querySelector('tr.analysis-sort-row-ascend')).toBeInTheDocument()
  expect(
    screen
  ...
  expect(document.querySelector('tr.analysis-sort-row-descend')).toBeInTheDocument()
  expect(
    screen
      .getAllByRole('button', { name: /查看.+的全部时段数据/ })
      .map((button) => button.textContent),
